...
const GameBoard = ({ randomWords, randomClues, wordsToGuess, players, currentUser, roomCode, currentGameID, currentRound, isSubmitted, setIsSubmitted, hasEveryoneSubmitted, setHasEveryoneSubmitted, showScoreBoard, playerScoresPerRound, setIsGameStarted }) => {
    ...
    const submittedButtonCSS = useRef(`bg-green-400 hover:cursor-not-allowed w-56 mt-4 hover:ring-cyan-500 hover:ring-1 hover:shadow-md text-white font-bold py-2 px-4 rounded focus:outline-none transition-all`);
    const readyForSubmissionButtonCSS = useRef(`bg-primary dark:bg-primaryDark w-56 mt-4 hover:ring-cyan-500 hover:ring-1 hover:shadow-md text-white font-bold py-2 px-4 rounded focus:outline-none transition-all`);
    const notReadyForSubmissionButtonCSS = useRef(`bg-neutral hover:cursor-not-allowed dark:bg-primaryDark w-56 mt-4 hover:ring-cyan-500 hover:ring-1 hover:shadow-md text-white font-bold py-2 px-4 rounded focus:outline-none transition-all`);
                        ...
                        <>
                            ...
                            {hasEveryoneSubmitted && (
                                <div className="flex justify-between">
                                    <button onClick={handleNextRound} className="bg-primary dark:bg-primaryDark w-56 mt-4 hover:ring-cyan-500 hover:ring-1 hover:shadow-md text-white font-bold py-2 px-4 rounded focus:outline-none transition-all" type="button">
                                        Next Round
                                    </button>
                                    <button onClick={handleEndGame} className="bg-neutral dark:bg-primaryDark w-56 mt-4 hover:ring-1 hover:shadow-md text-white font-bold py-2 px-4 rounded focus:outline-none transition-all" type="button">
                                        End Game
                                    </button>
                                </div>
                            )}
